...
namespace Microsoft.Spark.CSharp.Examples
{
	...
	class SparkCLREventHubsExample
	{
		static void Main(string[] args)
		{
			...
			StreamingContext sparkStreamingContext = StreamingContext.GetOrCreate(checkpointPath,
				() =>
				{
					...
					countByLogLevelAndTime.ForeachRDD(countByLogLevel => new SparkCLREventHubsExampleHelper().Execute(countByLogLevel));
		...
		public class SparkCLREventHubsExampleHelper
		{
			public void Execute(RDD<string> rdd)
			{
				var dimensionalCountCollection = rdd.Collect();
